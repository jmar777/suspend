...
Suspend.prototype.handleYield = function handleYield(result) {
	...
	// bail out if no yielded value is given
	// (assume resume will be passed manually)
	if (!resumable) return;
...
Suspend.prototype.resume = function resume(err) {
	if (this.done) {
		throw new Error('Resume invoked after generator was complete.'
			+ ' Check for multiple callbacks.');
	}
	...
	// temporary backwards compatibility for .send(val) instead of .next(val)
	// in node 0.11.2
